...
```python
## Load individual example tracked cells and plot
```

```python
## Plot some example cells from all 3 sessions
...
sess1 = ut.load_sess_pickle(path_dict['preprocessed_root'],mouse,exp_day=5)
sess2 = ut.load_sess_pickle(path_dict['preprocessed_root'],mouse,exp_day=8)
sess3 = ut.load_sess_pickle(path_dict['preprocessed_root'],mouse,exp_day=10)
```
...
for sess in (sess1, sess2, sess3):
    ...
    sess.rm_timeseries('dff')
    sess.add_timeseries(dff=dFF, events=events) 
    sess.add_pos_binned_trial_matrix(['dff','events'],'pos',
                                    bin_size=10, 
                                    min_pos = 0,
                                    max_pos=450
                                    )
...
## Plot ROIs of interest

```python
from InVivoDA_analyses import placeCellPlot
```

```python
da.plotUtils.set_fig_params(fontsize=8)
```

```python
da.plotUtils.set_fig_params(fontsize=8)

COI = 334 # cell of interest, currently from GD7
col = np.where(common_rois[1,:]==COI)[0][0]
...
for c_i, c in enumerate(common_rois[:, col]):
    ...
    if c_i == 0:
        sess = sess1
        ref_mat = sess.trial_matrices['events'][0][:,:,c]
    ...
    targ_mat = sess.trial_matrices['events'][0][:,:,c]
            
    norm_targ_mat = targ_mat / use_norm
            
    pf = ut.nansmooth(norm_targ_mat, 1, axis=1)

    rzone, _ = da.behavior.get_reward_zones(sess)
    ...
    # fig = placeCellPlot.plot_all_single_cells(sess,c,
    #                                           ts_key='events',
    #                                 normalization_method='mean',
    #                                plot_reward_zone = True,
    #                                          )

    roi_com = [sess.s2p_stats[c]['ypix'].mean(), sess.s2p_stats[c]['xpix'].mean()]
    ...
    ax[c_i+common_rois.shape[0]].imshow(sess.s2p_ops['meanImg'][ybounds[0]:ybounds[1],xbounds[0]:xbounds[1]],cmap='Greys_r',aspect='auto')
    roi = np.zeros([512,796*sess.n_planes])*np.nan
    roi[sess.s2p_stats[c]['ypix'], sess.s2p_stats[c]['xpix']]=1
    ax[c_i+common_rois.shape[0]].imshow(roi[ybounds[0]:ybounds[1],xbounds[0]:xbounds[1]],cmap='cool',alpha=.65) #bwr_r
    ax[c_i+common_rois.shape[0]].set_title(f"cell {c}, day {c_i}", fontsize=8)
...
save_figure = False
if save_figure:
    fig_dir = da.utilities.make_fig_dir(path_dict)
    da.plotUtils.savefig(fig, fig_dir, ("%s_expdays5-8-10_TRACKED_cell_GD7-%dpf_roi_cyan" % (
        mouse, COI)),
              extension=".pdf")
