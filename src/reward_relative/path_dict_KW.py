...
DATA_ROOT = os.path.join(HOME, "SosaRepo","Data")  # parent path to data
PP_ROOT = DATA_ROOT # path to preprocessed data
SBX_ROOT = os.path.join("/media","marielenasosa","T7","2p_raw_data") # scanbox data path, if different from preprocessed data path
GDRIVE_ROOT = os.path.join("/mnt","gdrive","2P_Data") # alternate raw data path - MetaLearn_GDrive: mounted via rclone

GIT_ROOT = os.path.join(HOME,"gitrepos")
